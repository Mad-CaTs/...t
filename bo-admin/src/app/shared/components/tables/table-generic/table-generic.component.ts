...
export class TableGenericComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  onRowSelect(row: any) {
    // Si todos están seleccionados, marcar selectAll
    if (this.effectiveCheckboxMode !== 'select') return;
    this.selectAll = this.data.length > 0 && this.data.every((r) => !!r.selected);
  }
  ...
  get isIndeterminate(): boolean {
    ...
    const sel = this.data.filter(r => !!r?.selected).length;
