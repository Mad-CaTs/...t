...
@Component({
  ...
  templateUrl: './type-entry.component.html',
  styleUrls: ['./type-entry.component.scss']
})
export class TypeEntryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.routerEventsSub = this.router.events.pipe(
      ...
      if (evt.urlAfterRedirects.includes('/type-entry')) {
        this.loadEntryTypes();
      }
    });
  }
  ...
  get pagedData(): EntryType[] {
    const start = (this.pageIndex - 1) * this.pageSize;
    return this.entryTypes.slice(start, start + this.pageSize);
  }
  ...
  onSearch(value: string) {
    ...
    if (!query) {
      this.entryTypes = [...this.allEntryTypes];
      this.pageIndex = 1;
      this.cdr.detectChanges();
      return;
    }

    this.entryTypes = this.allEntryTypes.filter(type => {
      const nameText = normalize(type.ticketTypeName);
  ...
  onSaveModal(data: { name: string; status: boolean }) {
    ...
    const isDuplicateCreate = !this.editMode && this.allEntryTypes.some(e =>
      e.ticketTypeName.trim().toLowerCase() === nameLower
    );
    const isDuplicateEdit = this.editMode && this.allEntryTypes.some(e =>
      e.ticketTypeId !== this.selectedToEdit?.ticketTypeId &&
      e.ticketTypeName.trim().toLowerCase() === nameLower
    );
    ...
    const request$ = this.editMode && this.selectedToEdit
      ? this.entryTypeService.update(this.selectedToEdit.ticketTypeId, {
        ticketTypeName: data.name,
        status: data.status
      })
      : this.entryTypeService.create({
        ticketTypeName: data.name,
        status: data.status
      });
    ...
    request$.pipe(
      ...
      next: () => {
        ...
        this.notifyMessage = this.editMode
          ...
          : 'El nuevo tipo de entrada ha sido registrado.';
        this.loadEntryTypes();
        this.showNotify = true;
      },
      error: (err) => {
  ...
  onConfirmDelete() {
    ...
    const name = this.selectedToDelete.ticketTypeName;
    this.showLoadingModal();

    this.entryTypeService.delete(this.selectedToDelete.ticketTypeId).pipe(
      finalize(() => this.hideLoadingModal())
    ).subscribe({
      next: () => {
        this.notifyType = 'success';
