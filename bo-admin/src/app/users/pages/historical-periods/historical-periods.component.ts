...
export class HistoricalPeriodsComponent {
	...
	onPeriodChange(): void {
		...
		if (selectedPeriod && selectedPeriod.text) {
			...
			this.periodService.getPeriodIdByDate(firstDate).subscribe((response) => {
				...
				if (selectedType === '1') {
					this.getHistoricoCompuestoData();
				} else if (selectedType === '2') {
					this.getHistoricoResidualData();
				}
			});
		}
	}
	...
	getHistoricoCompuestoData(): void {
		this.loading = true;

		this.periodService.getHistoricoCompuestoData(this.periodId).subscribe({
			next: (response) => {
	...
	getHistoricoResidualData(): void {
		this.loading = true;

		this.periodService.getHistoricoResidualData(this.periodId).subscribe({
			next: (response) => {
				if (response && response.data && response.data.length > 0) {
					this.table.data = [...response.data];
					this.noDataFound = false;
	...
	exportData(type: string, periodId: number): void {
		...
		const exportMethods: { [key: string]: (id: number) => Observable<Blob> } = {
			compuesto: this.periodService.exportHistoricCompoundData.bind(this.periodService),
			residual: this.periodService.exportHistoricResidualData.bind(this.periodService)
		};
