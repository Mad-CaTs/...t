...
export class PageInfoService {
  ...
  public updateBreadcrumbs(currentRoute: string) {
    this.setBreadcrumbs([]);
    ...
    MENU_ITEMS.forEach((menu) => {
      if (menu.subMenu) {
        menu.subMenu.forEach((sub) => {
          if (sub.link === currentRoute) {
            breadcrumbs.push({ title: menu.title, path: menu.link, isActive: false });
            breadcrumbs.push({ title: sub.title, path: sub.link, isActive: true });
          } else if (sub.subMenu) {
            sub.subMenu.forEach((subSub) => {
              if (subSub.link === currentRoute) {
                breadcrumbs.push({ title: menu.title, path: menu.link, isActive: false });
                breadcrumbs.push({ title: sub.title, path: sub.link, isActive: false });
                breadcrumbs.push({ title: subSub.title, path: subSub.link, isActive: true });
              }
            });
          }
        });
    ...
    if (breadcrumbs.length > 0) {
      const lastBreadcrumb = breadcrumbs[breadcrumbs.length - 1].title;
      this.setTitle(lastBreadcrumb);
