...
@Component({
	selector: 'app-modal-detail-car',
	standalone: true,
	imports: [CommonModule, ModalComponent, ReactiveFormsModule, FormControlModule],
	templateUrl: './modal-detail-car.component.html',
	styleUrls: ['./modal-detail-car.component.scss']
})
export class ModalDetailCarComponent {
	@Input() data: any;
	...
	public onReject() {
		...
		const modalRef = this.modalManager.open(ModalRejectCarComponent, { centered: true, size: 'md' });
		const modal = modalRef.componentInstance as ModalRejectCarComponent;
	}
