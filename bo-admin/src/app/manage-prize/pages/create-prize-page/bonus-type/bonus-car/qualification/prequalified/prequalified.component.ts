...
@Component({
  ...
  templateUrl: './prequalified.component.html',
  styleUrls: ['./prequalified.component.scss']
})
export class PrequalifiedComponent implements OnInit {
  ...
  extraButtons: FilterExtraButton[] = [
    ...
    { key: 'qualify', label: 'Calificar', variant: 'secondary', iconClass: 'bi bi-check2-square' }
  ];
  ...
  genericKeys: string[] = [
    'item', 'usuario', 'nombresApellidos', 'paisResidencia', 'rango', 'puntosAlcanzados', 'puntosMeta', 'fechaCierre', 'ciclosRecalificados', 'estado'
  ];
  genericWidths: string[] = [
    '5%', '8%', '15%', '10%', '10%', '10%', '10%', '12%', '12%', '10%'
  ];

  private allRows = PREQUALIFIED_MOCK.map(x => ({ ...x }));
  ...
  onModalConfirm() {
    ...
    } else if (t === 'qualify-confirm') {
      this.openModal('qualify-success', { title: 'Calificación aplicada', message: 'Se calificó correctamente.', iconType: 'success', oneButton: true, primaryText: 'Entendido' });
    }
  }
  ...
  getModalIcon(): 'success' | 'error' | 'info' | 'warning' {
    return (this.modalConfig && this.modalConfig.iconType) ? this.modalConfig.iconType : 'info';
  }
  ...
  onExtraAction(btn: FilterExtraButton) {
    ...
    if (btn.key === 'notify') {
      ...
      if (ids.length === 0) {
        ...
        this.openModal('notify-confirm', { title: 'Recuerda', message: `Al aceptar notificar, les llegará la notificación a los ids: ${idsText} de que suban sus proformas. Si estás seguro, puedes continuar; de lo contrario, se recomienda cancelar.`, iconType: 'warning', oneButton: false, primaryText: 'Notificar', secondaryText: 'Cancelar', closeOnConfirm: false });
      }
      return;
    }
    if (btn.key === 'qualify') {
      const ids = this.getSelectedIds();
      if (ids.length === 0) {
        this.openModal('require-selection', { title: 'Atención', message: 'Debes seleccionar al menos uno para usar Calificar.', iconType: 'info', oneButton: true, primaryText: 'Entendido' });
      } else {
        const idsText = ids.join(', ');
        this.openModal('qualify-confirm', { title: 'Recuerda', message: `Al aceptar calificar, se pasará a calificados a los ids: ${idsText}. Si estás seguro, puedes continuar; de lo contrario, se recomienda cancelar.`, iconType: 'warning', oneButton: false, primaryText: 'Calificar', secondaryText: 'Cancelar', closeOnConfirm: false });
      }
      return;
    }
  }
  ...
  onViewRow(row: any) {
    ...
    if (id != null) {
      ...
      this.router.navigateByUrl(`/dashboard/manage-prize/bonus-type/car/qualification/detail-partner-rating/${id}?from=prequalified`);
      return;
    }
  ...
  onDeleteRow(row: any) {
    alert(`Eliminar fila: ${row?.usuario || row?.id}`);
  }
  ...
  private applyFilters() {
    ...
    if (fRange && (!Array.isArray((fRange as any).options) || (fRange as any).options.length === 0)) {
      (fRange as any).options = Array.from(new Set(this.allRows.map(r => r.rango)))
        .sort()
    ...
    if (fEstado && (!Array.isArray((fEstado as any).options) || (fEstado as any).options.length === 0)) {
      (fEstado as any).options = Array.from(new Set(this.allRows.map(r => r.estado)))
        .sort()
        .map(v => ({ label: v, value: v }));
    }
  }
