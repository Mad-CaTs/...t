...
import { IPendingRequest, IValidatedRequest } from "@interfaces/legal-module.interface";

/** Genera un registro aleatorio de solicitud pendiente tipo Certificado o Contrato */
let numeroCounter = 1;
function generateMockPendingRequest(tipo: 'Certificado' | 'Contrato'): IValidatedRequest {
    return {
        ...
        status: faker.helpers.arrayElement(['Validado']),
    };
}

export const certificadosData: IValidatedRequest[] = Array.from({ length: 5 }, () =>
    generateMockPendingRequest('Certificado')
);

export const contratosData: IValidatedRequest[] = Array.from({ length: 5 }, () =>
