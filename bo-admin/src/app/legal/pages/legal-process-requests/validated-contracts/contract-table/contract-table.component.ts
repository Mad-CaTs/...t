...
export class ContractTableComponent {
	@Input() data: IValidatedContractRequestOne[] = [];
	@Output() actualizarTabla = new EventEmitter<void>();
	table: TableModel<IValidatedContractRequestOne>;
	selectedItem: any; //tipo correspondiente
	statusList: Array<{ id: number; statusCode: number; name: string; detail: string }> = [];
	...
	constructor(
		...
		this.table = this.tableService.generateTable<IValidatedContractRequestOne>({
			headers: [
				'Fec. de Solicitud',
				'Lugar de recojo',
				'Solicitante',
	...
	notificarEx() {
		...
		//simulacion
		alert(`Se notificó correctamente a ${this.selectedItem?.solicitante || 'el usuario'}.`);
		//reseto de formulario
		this.form = { status: '', observacion: '' };
	...
	onStatusChange(statusCode: number) {
		const selected = this.statusList.find((s) => s.statusCode === Number(statusCode));
		if (selected) {
			this.form.observacion = selected.detail || '';
		} else {
