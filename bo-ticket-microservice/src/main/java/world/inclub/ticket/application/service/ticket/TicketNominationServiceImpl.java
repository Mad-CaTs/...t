...
public class TicketNominationServiceImpl implements TicketNominationService {
    ...
    @Override
    public Mono<String> renominateTicket(List<UUID> ticketUuids) {
        return ticketRepositoryPort.findByTicketUuidIn(ticketUuids)
                .switchIfEmpty(Mono.error(new NotFoundException("No tickets found for the provided UUIDs")))
                .collectList()
                .flatMap(tickets -> {
                    List<Long> attendeeIds = tickets.stream()
                            .map(Ticket::getAttendeeId)
                    ...
                    List<Long> paymentIds = tickets.stream()
                            .map(Ticket::getPaymentId)
                            .distinct()
                            .toList();

                    Flux<Attendee> attendeesFlux = attendeeRepositoryPort.findAttendeesByIdIn(attendeeIds);

                    Flux<Payment> paymentsFlux = paymentRepositoryPort.findByIdIn(paymentIds);

                    return Mono.zip(attendeesFlux.collectList(), paymentsFlux.collectList())
                            .flatMap(tuple -> {
                                ...
                                return Flux.fromIterable(tickets)
                                        .flatMap(ticket -> {
                                            Attendee attendee = attendees.stream()
                                                    ...
                                                    .findFirst()
                                                    .orElseThrow(() -> new NotFoundException("Attendee not found for ticket UUID: " + ticket.getTicketUuid()));

                                            Payment payment = payments.stream()
                                                    .filter(p -> p.getId().equals(ticket.getPaymentId()))
                                                    .findFirst()
                                                    .orElseThrow(() -> new NotFoundException("Payment not found for ticket UUID: " + ticket.getTicketUuid()));
                                        ...
                                        .collectList()
                                        .flatMap(updatedTickets -> ticketRepositoryPort.saveAll(updatedTickets).collectList())
                                        .thenReturn("Tickets renominated successfully");
                            });
                });
    }
