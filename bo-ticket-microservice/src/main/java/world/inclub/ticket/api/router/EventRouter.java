...
public class EventRouter {
    ...
    public RouterFunction<?> routeEvent(EventHandler handler) {
        return RouterFunctions
                ...
                .andRoute(RequestPredicates.GET("/events/{id}/tickets/report")
                        .and(RequestPredicates.accept(MediaType.APPLICATION_JSON)),
                        request -> handler.getTicketReport(request).flatMap(entity ->
                                ServerResponse.status(entity.getStatusCode()).body(Mono.just(entity.getBody()), Object.class)));
    }
}