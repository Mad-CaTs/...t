...
public class EventHandler {
    ...
    private final GetTicketReportUseCase getTicketReportUseCase;
        ...
        return request.multipartData()
                .map(multiValueMap -> {
                    ...
                    if (multiValueMap.getFirst("isMainEvent") != null) {
                        dto.setIsMainEvent(
                                Boolean.parseBoolean(((FormFieldPart) multiValueMap.getFirst("isMainEvent")).value()));
                    }
                    if (multiValueMap.getFirst("ticketTypeId") != null) {
                        dto.setTicketTypeId(
                                Integer.parseInt(((FormFieldPart) multiValueMap.getFirst("ticketTypeId")).value()));
                    }
                    if (multiValueMap.getFirst("eventTypeId") != null) {
                        dto.setEventTypeId(
                                Integer.parseInt(((FormFieldPart) multiValueMap.getFirst("eventTypeId")).value()));
                    }
                    if (multiValueMap.getFirst("eventDate") != null) {
                        dto.setEventDate(
                                LocalDate.parse(((FormFieldPart) multiValueMap.getFirst("eventDate")).value()));
                    }
                    if (multiValueMap.getFirst("startDate") != null) {
                        dto.setStartDate(
                                LocalTime.parse(((FormFieldPart) multiValueMap.getFirst("startDate")).value()));
                    }
    ...
    public Mono<ServerResponse> getPublicEventsWithZones(ServerRequest request) {
        return ServerResponse.ok()
                .contentType(MediaType.APPLICATION_JSON)
                .body(getPublicEventsWithZonesUseCase.getPublicEventsWithZones(),
                        PublicEventWithZonesResponseDto.class);
    }
    ...
    public Mono<ResponseEntity<Object>> getTicketReport(ServerRequest request) {
        Integer eventId = Integer.valueOf(request.pathVariable("id"));

        return ResponseHandler.generateResponse(
                HttpStatus.OK,
                getTicketReportUseCase.getTicketReport(eventId),
                true
        ).onErrorResume(NotFoundException.class, e ->
                ResponseHandler.generateResponse(HttpStatus.NOT_FOUND,
                        Mono.just("Evento con id " + eventId + " no encontrado"),
                        false
                )
        ).onErrorResume(e ->
                ResponseHandler.generateResponse(HttpStatus.BAD_REQUEST,
                        Mono.just("Error al obtener el reporte de tickets: " + e.getMessage()),
                        false
                )
        );
    }
