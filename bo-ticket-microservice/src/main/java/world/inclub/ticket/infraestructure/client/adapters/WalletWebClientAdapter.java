...
public class WalletWebClientAdapter implements WalletRepositoryPort {
    ...
    public Mono<Boolean> registerTransferBetweenWallets(ProcessWalletPaymentDto request) {
        return walletWebClient.post()
                .uri("/api/v1/wallettransaction/transfer/between-wallets")
                .bodyValue(request)
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<DataResponse<Boolean>>() {})
                .map(DataResponse::data);
    }
