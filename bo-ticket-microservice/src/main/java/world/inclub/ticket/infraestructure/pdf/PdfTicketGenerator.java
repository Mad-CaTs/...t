    ...
    public Mono<byte[]> generatePdfForTicket(Ticket ticket, Attendee attendee, Payment payment) {
        return Mono.zip(
                ...
                .flatMap(tuple2 -> {
                    ...
                    TicketQrPayload ticketQrCode = TicketQrPayload.builder()
                            .ticketUuid(ticket.getTicketUuid())
                            .eventName(eventZone.getEventName())
                            .zoneName(zone.getZoneName())
                            .status(ticket.getStatus())
                            .attendee(TicketQrPayload.Attendee.builder()
                    ...
                    try {
                        String ticketJson = objectMapper.writeValueAsString(ticketQrCode);
                        String encryptedData = qrCryptoService.encryptJson(ticketJson, keyProvider.getSecretKey());
                        return generateQrService.generateQr(encryptedData)
                                .map(qrBytes -> Tuples.of(qrBytes, eventZone, documentType, zone));
                    } catch (Exception e) {
                        return Mono.<Tuple4<byte[], PublicEventWithZonesResponseDto, DocumentType, PublicEventWithZonesResponseDto.ZoneDetail>>error(e);
                    }
                })
