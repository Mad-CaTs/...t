...
public class TransferObservationListener {
    ...
    @KafkaListener(topics = { "transfer-observation" }, groupId = KafkaConstants.GROUP_ID
            + "-observation-envelope", containerFactory = "transferNotificationRawKafkaListenerContainerFactory")
    public void consumeRaw(String json, @Header(name = "kafka_receivedMessageKey", required = false) String key) {
        if (json == null || json.isBlank())
            return;
        try {
            log.info("[OBSERVATION-LISTENER-ENVELOPE] Received raw payload key={} length={}", key, json.length());
            ...
            if (data == null || data.isMissingNode() || data.isNull()) {
                log.debug("[OBSERVATION-LISTENER-ENVELOPE] No data/body node found. Skipping");
                return;
            }

            JsonNode tr = data.path("transferRequest");
            if (tr.isMissingNode() || tr.isNull()) {
                if (hasAnyField(data, "idTransferRequest", "id_transfer_request", "id_transfer_status",
                        "user_to_correo_electronico", "user_from_email", "user_to_nombre")) {
                    tr = data;
                    log.info(
                            "[OBSERVATION-LISTENER-ENVELOPE] detected flat transferRequest fields at data level; treating data as transferRequest");
                }
            }

            if (tr.isMissingNode() || tr.isNull()) {
                log.debug("[OBSERVATION-LISTENER-ENVELOPE] transferRequest missing. Skipping");
                return;
            }
            ...
            String observationDetail = asText(obs != null ? obs.get("detailObservationTransfer") : null,
                    obs != null ? obs.get("detail") : null);
            Integer observationTypeId = asInt(obs != null ? obs.get("idTransferObservationType") : null);

            if (targetEmail == null || targetEmail.isBlank()) {
                log.warn("[OBSERVATION-LISTENER-ENVELOPE] No recipient email found in payload; skipping send.");
                return;
            }
            ...
            if (html == null || html.isBlank()) {
                log.error(
                        "[OBSERVATION-LISTENER-ENVELOPE] rendered HTML is null/blank. recipient={} subject={} template=transfer/email_transfer_observation.ftl model={}",
                        targetEmail, subject, builderAdapter.getModelForMessage(dto));
            }

            log.info(
                    "[OBSERVATION-LISTENER-ENVELOPE] Sending observation email to={} subject={} htmlLength={} idReq={}",
            ...
            transferNotificationService.sendTransferNotification(dto, subject, html)
                    .subscribe(sent -> {
                        if (sent)
                            log.info("[OBSERVATION-LISTENER-ENVELOPE] Email sent to {}", targetEmail);
                        else
                            log.error("[OBSERVATION-LISTENER-ENVELOPE] Email NOT sent to {}", targetEmail);
                    });

        } catch (Exception e) {
            log.error("[OBSERVATION-LISTENER-ENVELOPE] Failed to process raw payload. key={} err={} json={}", key,
                    e.getMessage(), json);
        }
    }
