...
import world.inclub.appnotification.bonusrewards.infrastructure.email.enums.PaymentEmailTemplateEnum;
...
@Component
public class PaymentEmailBuilderAdapter implements EmailTemplateBuilderPort<PaymentNotificationMessage> {
    ...
    private static String getPaymentPendingReviewTemplate(PaymentNotificationMessage message) {
        return EmailTemplateLoader.loadTemplate(PaymentEmailTemplateEnum.PAYMENT_PENDING_REVIEW.getTemplatePath())
    ...
    private static String getPaymentCompletedTemplate(PaymentNotificationMessage message) {
        return EmailTemplateLoader.loadTemplate(PaymentEmailTemplateEnum.PAYMENT_COMPLETED.getTemplatePath())
                .replace("{{logoUrl}}", EmailConstants.LOGO_WHITE_URL)
                .replace("{{fullName}}", message.user().fullName())
                .replace("{{paymentType}}", message.payment().paymentType())
                .replace("{{paymentSubType}}", message.payment().paymentSubType())
    ...
    private static String getPaymentRejectedTemplate(PaymentNotificationMessage message) {
        return EmailTemplateLoader.loadTemplate(PaymentEmailTemplateEnum.PAYMENT_REJECTED.getTemplatePath())
                .replace("{{logoUrl}}", EmailConstants.LOGO_WHITE_URL)
                .replace("{{fullName}}", message.user().fullName())
    ...
    public String getSubject() {
        return switch (lastStatus) {
            case COMPLETED -> PaymentEmailTemplateEnum.PAYMENT_COMPLETED.getSubject();
            case PENDING_REVIEW -> PaymentEmailTemplateEnum.PAYMENT_PENDING_REVIEW.getSubject();
            case REJECTED -> PaymentEmailTemplateEnum.PAYMENT_REJECTED.getSubject();
