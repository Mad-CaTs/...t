...
import { SimpleTableComponent } from 'src/app/guest/pages/my-purchases/commons/component/simple-table/simple-table.component';
import { PaginationComponent } from '../../../../../guest/pages/my-purchases/commons/component/pagination/pagination.component';
import { PurchasesService } from 'src/app/guest/commons/services/purchases.service';
import { NominationBatchRequest, NominationService } from 'src/app/guest/commons/services/nomination.service';
import { TicketsService } from 'src/app/guest/commons/services/tickets.service';
import { Purchases, PurchasesResponse } from 'src/app/guest/commons/interfaces/guest-components.interface';
import { PurchasesColumns } from 'src/app/guest/commons/constants/pages/purchases';
import { RejectedColumns } from 'src/app/guest/commons/constants/pages/rejected';
import { RestorePuchasesModalComponent } from 'src/app/guest/pages/my-purchases/commons/component/restore-puchases-modal/restore-puchases-modal.component';
import { NominationsColumns } from 'src/app/guest/commons/constants/pages/nomination';
...
import { getProfileTabs } from './commons/get-profile-tabs';
...
@Component({
	...
	templateUrl: './ambassador-purchases.component.html',
	styleUrl: './ambassador-purchases.component.scss'
})
export class AmbassadorPurchasesComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		this.profileTabs = getProfileTabs(this.setActiveTab.bind(this), this.activeTab);
		...
		this.route.queryParams.pipe(takeUntil(this.destroy$)).subscribe((params) => {
			...
			if (open && paymentId) {
				...
				this.router.navigate([], {
					queryParams: { openNomination: null, paymentId: null, ticketUuid: null },
					replaceUrl: true
				});
	...
	private getNominations(page?: number, size?: number) {
		...
		this.nominationService.getNominationsStatus(p, s, userId)
			.subscribe({
				...
				error: (err) => {
					// fallback estático para desarrollo
					...
					this.emptyNominations = {
						...
						message: 'Actualmente no existen usuarios disponibles para nominar en este evento.'
					};
					if (err?.status === 500) {
		...
		if (response.result && response.data) {
			const allPurchases = response.data.content
				? response.data.content.filter(
						(p) => p.status !== 'REJECTED' && p.status !== 'TEMPORAL_REJECTED'
				  )
				: [];
			...
			this.rejected = response.data.content
				? response.data.content.filter(
						(i) => (i.status === 'REJECTED' || i.status === 'TEMPORAL_REJECTED') && (i.isDiscounted == null)
					)
				: [];
	...
	changePage(pageNumber: number): void {
		if (pageNumber >= 0 && pageNumber < this.totalPages && pageNumber !== this.currentPage.value) {
	...
	translatePaymentStatus(status: string) {
	  ...
	  nominationHeaders = NominationsColumns.map(col => col.header);


/*   	nominationsColumns = NominationsColumns;
 *//*   get headers(): string[] {
    return NominationsColumns.map(c => c.header); 
  } */

  get headers() {
		return ['N° de orden', 'Fecha de compra	', 'Evento', 'Medio de pago	', 'Total', 'Estado'];
	}

	get headersRejected(): string[] {
  return ['N° de orden', 'Fecha de compra', 'Evento', 'Medio de pago', 'Total', 'Estado','Acciones'];
}

/* get nominationsColumns
 */
	...
	openNominationForTicket(ticket: any) {
		...
		this.router.navigate(['/profile/ambassador/events/my-purchases', paymentId, 'nomination'], {
	...
	submitNomination() {
		console.log(
			'submitNomination invoked, valid:',
			this.nominationForm.valid,
			'value:',
			this.nominationForm.value
		);
		...
		let ticketUuid =
			(this.selectedPurchase as any).ticketUuid ?? (this.selectedPurchase as any).tickets?.[0]?.uuid;
		...
		if ((!paymentId || !ticketUuid) && (this.selectedTicketUuid || this.selectedPaymentId)) {
			...
			console.log('Fallback desde propiedades selectedPayment/selectedTicket:', {
				paymentId,
				ticketUuid
			});
		}
		...
		if (!paymentId || !ticketUuid) {
			console.warn('paymentId o ticketUuid incompletos', {
				paymentId,
				ticketUuid,
				selected: this.selectedPurchase
			});
			...
			if (paymentId && !ticketUuid) {
				...
				if (getDetails) {
					getDetails(Number(paymentId), 0, this.pageSize).subscribe({
						next: (resp: any) => {
							...
							if (Array.isArray(content) && content.length === 1) {
								foundUuid =
									content[0].ticketUuid ?? content[0].uuid ?? content[0].ticketId ?? null;
							} else if (Array.isArray(content) && content.length > 0) {
								const notNominated = content.find(
									(t: any) =>
										!String(t.status || '')
											.toUpperCase()
											.includes('NOMINAT')
								...
								foundUuid =
									notNominated?.ticketUuid ??
									notNominated?.uuid ??
									notNominated?.ticketId ??
									null;
								if (!foundUuid) {
									foundUuid =
										content[0].ticketUuid ??
										content[0].uuid ??
										content[0].ticketId ??
										null;
								}
							...
							if (foundUuid) {
								...
								console.error('No se pudo derivar ticketUuid desde el detalle de tickets', {
									paymentId,
									resp
								});
							}
						...
						error: (err: any) => {
							console.error(
								'Error al obtener detalle de tickets para derivar ticketUuid:',
								err
							);
						}
					});
					...
					console.error(
						'ticketsService.getTicketsDetails no está disponible para derivar ticketUuid'
					);
				}
			}
	...
	private _sendNominationRequest(paymentId: number, ticketUuid: string, formValue: any) {
		...
		if (returnUrl) {
			...
			// fallback por si no había returnUrl
			this.router.navigate(['/profile/ambassador/events/my-purchases']);
		}
	}
	...
	getNominationClass(estado: string): string {
		...
		// Verificar primero los casos específicos
		...
		// Para otros casos que contengan 'nominat'
