...
import { DirectUsersService } from '../../services/direct-users.service';
...
import { IUserOption } from '../../interfaces/direct-users.interface';
import { typeUserOptions } from '../../mocks/mock';

@Component({
	selector: 'app-table-account-tree-direct-users',
	templateUrl: './table-account-tree-direct-users.component.html',
	standalone: true,
	imports: [CommonModule, TableComponent, DropdownModule, InputComponent, SelectComponent],
	styleUrls: []
})
export class TableAccountTreeDirectUsersComponent implements OnInit, OnChanges, OnDestroy, AfterViewChecked {
	...
	typeUserOptions: IUserOption[] = typeUserOptions;
	@Output() selectedValueChange = new EventEmitter<string>();
	@Output() typeUserChanged = new EventEmitter<string>();
	...
	/* typeUserOptions = [
		{ content: 'directos', value: '1' },
		{ content: 'liquidacion', value: '2' }
	  ]; */
	...
	ngOnInit(): void {
		...
		this.form.get('typeUser')?.valueChanges.subscribe((value) => {
			this.typeUserChanged.emit(value);
		});
