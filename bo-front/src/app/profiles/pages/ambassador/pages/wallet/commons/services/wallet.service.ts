...
export class WalletService {
	...
	getAllListWalletById(id: number, page: number, size: number, search: string): Observable<any> {
		...
		return this.http.get<any>(`${this.urlWalletDirecto}/wallettransaction/list/${id}`, { params }).pipe(
			map((response: any) => {
				return response;
			})
		);
	}
	getAllListWalletRecharge(id: number, page: number, size: number, search: string): Observable<any> {
		let params = new HttpParams()
			.set('page', page.toString())
			.set('size', size.toString());
		if (search) {
		...
		return this.http.get<any>(`${this.urlWalletDirecto}/wallettransaction/list/recharge/${id}`, { params }).pipe(
			map((response: any) => {
				return response;
			})
		);
	}
	getAllListWalletTransaction(id: number, page: number, size: number, search: string): Observable<any> {
		let params = new HttpParams()
			.set('page', page.toString())
			.set('size', size.toString());
		if (search) {
		...
		return this.http.get<any>(`${this.urlWalletDirecto}/wallettransaction/list/transfers/${id}`, { params }).pipe(
			map((response: any) => {
				return response;
			})
		);
	...
	postGenerateTokenGestionBancaria(idUser: any): Observable<any> {
		return this.http.post<any>(`${this.urlWallet}/tokenwallet/create-gestion-bancario/${idUser}`, {});
	}
	verifyValidityTokenGestionBancario(body: TokenRequest): Observable<any> {
		return this.http.post<any>(`${this.urlWallet}/tokenwallet/validate-bancario`, body);
	}
	...
	postSolicitTransferBank(file: File, data: any): Observable<any> {
	 ...
	 getWalletMovements(id: number): Observable<Blob> {
     return this.http.get(`${this.urlWallet}/wallet/movements/${id}/excel`, {
         responseType: 'blob'
     });
    }
