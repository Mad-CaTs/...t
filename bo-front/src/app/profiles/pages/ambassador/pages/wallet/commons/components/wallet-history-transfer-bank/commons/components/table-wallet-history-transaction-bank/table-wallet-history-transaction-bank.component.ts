...
//import { IWalletHistoryTransactionTable } from '../../../../../../../payment-detail/commons/interfaces/payments-and-comissions.interfaces';
...
import { ModalWalletDetailRequestBankTransferBankComponent } from '../../modals/modal-wallet-detail-request-transfer/modal-wallet-detail-request-bank-transfer.component';
...
@Component({
	selector: 'app-table-wallet-history-transaction-bank',
	templateUrl: './table-wallet-history-transaction-bank.component.html',
	standalone: true,
	imports: [CommonModule, PaginationComponent, TableComponent],
	providers: [DatePipe],
	styleUrls: []
})
export class TableWalletHistoryTransactionBankComponent implements OnInit, OnChanges, OnDestroy {
	...
	@Output() requestTransferCompleted = new EventEmitter<void>();
	...
	public onRequestTransfer() {
		const modalRef = this.modal.open(ModalWalletDetailRequestBankTransferBankComponent, {
			centered: true
		});
		...
		modalRef.componentInstance.resultEmitter.subscribe((result: string) => {
			if (result === 'success') {
				// this.refreshTable();
				this.requestTransferCompleted.emit();
			} else {
