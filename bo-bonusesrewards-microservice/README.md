...
```bash
curl -X GET /api/v1/car-bonus/documents-types
```
...
```bash
curl -X GET /api/v1/car-bonus/documents-types/1
```
...
```bash
curl -X GET "/api/v1/car-bonus/documents-types/search?name=cedula"
```
...
```bash
curl -X POST /api/v1/car-bonus/documents-types \
  -H "Content-Type: application/json" \
  -d '{"name": "Pasaporte"}'
```
...
```bash
curl -X PUT /api/v1/car-bonus/documents-types1 \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE /api/v1/car-bonus/documents-types/1
```
...
#### GET - Obtener información extra de asignación de auto
```bash
